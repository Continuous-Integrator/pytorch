...
def add_push_null(
    ...
    def inst_has_bit_set(idx: int) -> bool:
        if insts[idx].arg is None:
            raise AssertionError(
                f"expected insts[{idx}].arg to be not None"
            )
    ...
    def _update(byteno_delta: int, lineno_delta: int) -> None:
        while byteno_delta != 0 or lineno_delta != 0:
            ...
            if byte_offset == 0 and line_offset == 0:
                raise AssertionError(
                    "byte_offset and line_offset cannot both be 0"
                )
        ...
        if inst.opcode in jumps:
            if inst.opcode not in dis.hasjabs:
                ...
                if inst.target.offset is None:
                    raise AssertionError(
                        f"jump target for {inst.opname} has no offset"
                    )
            ...
            if inst.opcode in dis.hasjabs:
                if sys.version_info < (3, 11):
                    ...
                    if target.offset is None:
                        raise AssertionError(
                            "absolute jump target has no offset"
                        )
                ...
                if target.offset is None:
                    raise AssertionError(
                        "relative jump target has no offset"
                    )
                if inst.offset is None:
                    raise AssertionError(
...
def check_inst_exn_tab_entries_valid(instructions: list[Instruction]) -> None:
    ...
    for i, inst in enumerate(instructions):
        if inst.exn_tab_entry:
            if sys.version_info < (3, 11):
                raise AssertionError(
                    "exn_tab_entry is only supported in Python 3.11+"
                )
...
def check_offsets(instructions: Sequence[Instruction]) -> None:
    ...
    for inst in instructions:
        if inst.offset != offset:
            raise AssertionError(
                f"instruction {inst.opname} has offset {inst.offset}, "
                f"expected {offset}"
            )
...
def fix_vars(
    ...
    if sys.version_info < (3, 11):
        if varname_from_oparg is not None:
            raise AssertionError(
                "varname_from_oparg should be None for Python < 3.11"
            )
        ...
        if not callable(varname_from_oparg):
            raise AssertionError(
                "varname_from_oparg must be callable for Python 3.11+"
            )
        allnames = {}
    ...
    for i in range(len(instructions)):
        ...
        if instructions[i].opname == "LOAD_GLOBAL":
            ...
            if instructions[i].argval is _NotProvided:
                raise AssertionError(
                    "LOAD_GLOBAL instruction must have argval set"
                )
            if sys.version_info >= (3, 11):
            ...
            if instructions[i].argval is _NotProvided:
                raise AssertionError(
                    "LOAD_ATTR instruction must have argval set"
                )
            if sys.version_info >= (3, 12):
                if instructions[i].arg is None:
                    raise AssertionError(
            ...
            if instructions[i].arg is None:
                raise AssertionError(
                    "LOAD_SUPER_ATTR instruction must have arg set"
                )
            if instructions[i].argval is _NotProvided:
                raise AssertionError(
                    "LOAD_SUPER_ATTR instruction must have argval set"
                )
