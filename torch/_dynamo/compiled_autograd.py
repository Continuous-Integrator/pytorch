    ...
    def prep_with_graph(self, graph: torch.fx.Graph) -> None:
        ...
        for node in acc_grad_nodes:
            ...
            if param_node.args[0] is not inputs_node:  # type: ignore[possibly-undefined]
                raise AssertionError(
                    "Expected param_node.args[0] to be inputs_node"
                )
...
class AutogradCompilerInstance:
    ...
    def proxy_call_aot_backward(
        ...
        def copy_paste_aot_backward_graph() -> list[torch.Tensor]:
            ...
            if poutputs is None:
                raise AssertionError(
                    "No output node found in backward graph"
                )
    ...
    def post_acc_grad_hook(
        ...
        if not isinstance(input, torch.Tensor):
            raise AssertionError(
                f"Expected torch.Tensor, got {type(input)}"
            )
        if self.hooks_proxy is None:
    ...
    def move_graph_nodes_to_cuda(self, graph: torch.fx.Graph) -> list[int]:
        ...
        if nodes[first_getitem_idx] != inputs_users[0]:
            raise AssertionError(
                "First getitem node does not match first inputs user"
            )
        last_getitem_idx = first_getitem_idx + len(inputs_users) - 1
        if nodes[last_getitem_idx] != inputs_users[-1]:
            raise AssertionError(
                "Last getitem node does not match last inputs user"
            )
    ...
    def to_proxy(self, t: Any) -> Any:
        ...
        if not isinstance(proxy_tensor, torch.fx.experimental.proxy_tensor._ProxyTensor):
            raise AssertionError(
                f"Expected _ProxyTensor, got {type(proxy_tensor)}"
            )
...
def reset() -> None:
    ...
    if in_compiled_autograd_region:
        raise AssertionError(
            "Cannot reset while in_compiled_autograd_region is True"
        )
    torch._C._dynamo.compiled_autograd.set_autograd_compiler(None, False)
    ...
    for i in range(len(needs_input_grad)):
        if needs_input_grad[i]:
            ...
            if i == 0:
                to_copy = res[i]
                if to_copy is None:
                    raise AssertionError("res[0] must not be None for needs_input_grad[0]")
