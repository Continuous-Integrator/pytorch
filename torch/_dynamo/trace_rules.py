...
def add_module_init_func(name: str, init_func: Callable[[], None]) -> None:
    ...
    if name in _lazy_module_init:
        raise AssertionError(
            f"Module init function already registered for {name}"
        )
...
def check_verbose(
    ...
    if isinstance(
        ...
        if filename is None:
            raise AssertionError(
                f"getfile returned None for function/method {obj}"
            )
        fi = FunctionInfo(
            obj,
            obj.__name__,
        ...
        if filename is None:
            raise AssertionError(
                f"getfile returned None for {type(obj)}"
            )
    ...
    if issubclass(
        ...
        if rule != SkipFunctionVariable:
            raise AssertionError(
                f"expected SkipFunctionVariable, got {rule}"
            )
