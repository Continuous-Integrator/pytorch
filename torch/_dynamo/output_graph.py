...
class OutputGraph(OutputGraphCommon):
    ...
    def add_backward_state_hook(
        ...
        if name in self.backward_state:
            raise AssertionError(f"backward_state hook name '{name}' already registered")
        ...
        try:
            if prior_tracer:
                ...
                if prior_tracer.parent is not self.current_tracer:
                    raise AssertionError("prior_tracer.parent must be the current tracer to preserve lineage")
    ...
    def register_attr_or_module(
        ...
        **options: Any,
        ...
        if isinstance(source, ParamBufferSource):
            raise AssertionError(f"source must not be a ParamBufferSource, got {source}")
        ...
        if isinstance(target, torch.Tensor):
            ...
            def wrap_name(module_key: str) -> VariableTracker:
                ...
                if "tensor_dict" in vt.as_proxy().node.meta:
                    raise AssertionError("'tensor_dict' already exists in proxy node meta")
            ...
            if not isinstance(target, torch.nn.Module):
                raise AssertionError(f"target must be a torch.nn.Module, got {type(target)}")
    ...
    def _get_stack_values_to_restore(
        ...
        for k, v in tx.symbolic_locals.items():
            ...
            if sys.version_info >= (3, 12):
                ...
                if type.__instancecheck__(NullVariable, v):
                    raise AssertionError("variable should never be NULL in Python < 3.12")
    ...
    def compile_subgraph(
        ...
        if not config.nested_graph_breaks:
            ...
            if self.root_tx is not tx:
                raise AssertionError("root_tx must be tx when nested_graph_breaks is disabled")
        ...
        while cur_tx is not None:
            ...
            if not all(block.can_restore() for block in cur_tx.block_stack):
                raise AssertionError("all blocks in cur_tx.block_stack must be restorable")
            ...
            if (
                ...
                if (
                    ...
                    if not isinstance(
                        ...
                        raise AssertionError(
                            "flat_returns must be a ListVariable"
                        )

                    vt_to_graph_out_idx: dict[VariableTracker, int] = {}
                    for value in pass2.graph_outputs.values():
                        if not isinstance(value, torch._dynamo.codegen.GraphOutputEntry):
                            raise AssertionError(
                ...
                for cell in freevars:
                    if cur_tx is self.root_tx:  # root frame
                        ...
                        if not cur_tx.post_prune_cell_and_freevars:
                            raise AssertionError("cur_tx.post_prune_cell_and_freevars must be set for nested frames")
        ...
        if self.backward_state:
            ...
            for name, val in self.backward_state.items():
                ...
                if self.backward_state_var is None:
                    raise AssertionError("backward_state_var must not be None when backward_state is set")
    ...
    def compile_and_call_fx_graph(
        ...
        with torch._guards.TracingContext.clear_frame():
            ...
            if not self.should_exit:
                raise AssertionError("should_exit must be True before call_user_compiler")
...
class SubgraphTracer(fx.Tracer):
    ...
    def create_graph_input(
        ...
        if self.is_export and self.parent is None:
            if source is None:
                raise AssertionError("source must not be None for export root tracer inputs")
    ...
    def track_produced_symints(
        ...
        def _proxy_with_example_value(
            ...
            if not isinstance(e_proxy, torch.fx.Proxy):
                raise AssertionError(f"e_proxy must be an fx.Proxy, got {type(e_proxy)}")
            ...
            if self.parent is not None:
                ...
                for s0 in self_to_be_bound:
                    ...
                    if not isinstance(example_val, torch.SymInt):
                        raise AssertionError(f"example_val must be a SymInt, got {type(example_val)}")
