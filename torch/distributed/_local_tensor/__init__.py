...
class LocalTensorMode(TorchDispatchMode):
    ...
    def __init__(self, ranks: int | frozenset[int]):
        ...
        # Used to store patched ProcessGroup class methods (.rank(), .size())
        self._old_pg_methods: dict[str, object] | None = None
    ...
    def _patch_dist(self) -> None:
        ...
        # Patch ProcessGroup.rank() and .size() at the class level so that
        # pg.rank() also returns a multi-rank SymInt inside LocalTensorMode.
        if self._old_pg_methods is not None:
            raise AssertionError
        PG = type(dist.distributed_c10d._get_default_group())
        self._old_pg_methods = {
            "rank": PG.__dict__["rank"],
        }
        PG.rank = lambda self: _LocalDist.get_rank(
            self
    ...
    def _unpatch_dist(self) -> None:
        ...
        pg_saved, self._old_pg_methods = self._old_pg_methods, None
        if pg_saved is None:
            raise AssertionError
        PG = type(dist.distributed_c10d._get_default_group())
        for name, descriptor in pg_saved.items():
            setattr(PG, name, descriptor)
