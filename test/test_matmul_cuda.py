...
class TestMatmulCuda(InductorTestCase):
    ...
    def test_grouped_gemm_2d_2d(self, strided, a_row_major, b_row_major, backend, dtype):
        if backend == "cublaslt" and torch.cuda.get_device_capability()[0] not in [10, 11]:
    ...
    def scaled_grouped_gemm_cublaslt_helper(self, op, fast_accum):
        ...
        self.assertEqual(C.reshape(C_ref.shape), C_ref, atol=1e-2, rtol=1e-2)

    @unittest.skipIf(TEST_WITH_ROCM, "ROCm doesn't support cuBLASLt grouped GEMM")
    @unittest.skipIf(torch.cuda.get_device_capability()[0] not in [10, 11], "cublaslt grouped gemm requires SM 10.x or 11.0")
    @parametrize("op", ["2d/2d", "2d/3d", "3d/2d", "3d/3d"])
    @parametrize("fast_accum", [False, True])
    def test_scaled_grouped_gemm_cublaslt_2d_3d(self, op, fast_accum):
        self.scaled_grouped_gemm_cublaslt_helper(op, fast_accum)

    @unittest.skipIf(TEST_WITH_ROCM, "ROCm doesn't support cuBLASLt grouped GEMM")
    @unittest.skipIf(torch.cuda.get_device_capability()[0] not in [10, 11], "cublaslt grouped gemm requires SM 10.x or 11.0")
    @parametrize("mode", ["default", "reduce-overhead"])
    def test_scaled_grouped_gemm_cublaslt_compiled(self, mode):
        device = "cuda"
        dtype = torch.float8_e4m3fn
        ngroups = 5
        m, n, k = 16, 16, 32
        ...
        C_ref = f_ref(A, B, scale_a, scale_b, use_fast_accum=True)
        C = f(A, B, scale_a, scale_b, use_fast_accum=True)
        self.assertEqual(C, C_ref)
